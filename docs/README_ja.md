...
### モード A — 画像サブフォルダを含むディレクトリ（従来の動作）

第一階層の各サブフォルダが独立したバッチとして処理されます。
...
### モード B — 画像ファイルを直接含むディレクトリ（単一バッチモード）

選択したディレクトリ直下に画像ファイルが存在する場合、すべての画像を単一バッチとして直接処理します。サブディレクトリは無視されます。

**入力**
<pre>
/Volumes/漫画フォルダ/CITY HUNTER Vol.xx/
├── page001.jpg
├── page002.jpg
└── page003.jpg ...
</pre>
...
/Volumes/漫画フォルダ/完了/
├── CITY HUNTER Vol.xx/
│   ├── CITY.HUNTER.CE.1-1.jpg  （しきい値を超えた場合は分割、右側が先に命名）
│   ├── CITY.HUNTER.CE.1-2.jpg  （左側）
│   └── CITY.HUNTER.CE.2.jpg    （しきい値を超えない場合は直接変換）
├── One Piece Vol.xx/
│   └── ...
└── Comic 3/
    └── ...
...
処理済みの画像は、[Me2Press](https://github.com/DawnLiExp/Me2Press) を使用して mobi 漫画ファイルにパックされています。
